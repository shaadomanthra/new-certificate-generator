<form action="edit_default_template" method="POST">
    ...
    <h4 class="m-0 mt-3 mb-1 text-danger">Logo</h4>
    <div class="mt-2">
        <select class="custom-select bg-dark text-white" name="logo">
            ...
            @php
                    ...
                    $l = $l[0];
                @endphp
                <option value="{{$logo}}" {{$file->logo == $logo ? "selected" : ""}}>{{$l}}</option>
                @endforeach
        </select>
        <h6 class="my-2"><span class="text-danger">Note: </span> Leave empty for default logo</h6>
    </div>
